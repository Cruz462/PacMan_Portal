...
def image_helper(color, direction):
    return [pg.image.load('images/pm_ghost' + color + direction + str(x) + '.png') for x in range(3, 5)]
...
class Ghost(Character):
    def __init__(self, game, v, grid_pt, grid_pt_next, timer, name="anonymous ghost"):
        super().__init__(game, v=v, grid_pt=grid_pt, grid_pt_next=grid_pt_next, name=name, scale=1.0, scale_factor=1.0)
        self.timer = timer
        self.look_down();
